...
Picking those twenty well requires weighing a portfolio of constraints simultaneously, since a peptide program that delivers on activity but fails on toxicity, stability, or developability is not a lead \citep{yan2022review}.
...
That output shape is a mismatch for the shortlisting decision a program director actually makes.
...
Data, oracles, and metrics are read-only, so the agent can only improve by writing a better ranking policy, not by reshaping the test.
The loop design is borrowed from \citet{karpathy2026autoresearch}; here it is directed for the first time at peptide candidate prioritization (see Section~\ref{sec:related_work} for its application in a companion study on molecular architectures).

The benchmark is public and modest in scope.
We assemble 3{,}554 antimicrobial peptides from DBAASP \citep{pirtskhalava2021dbaasp} with measured activity against \emph{E.~coli}, predicted toxicity from ToxinPred3 \citep{rathore2024toxinpred3}, predicted half-life from HLP \citep{sharma2014hlp}, and a rule-based developability score.
The agent ran 100 experiments over roughly one laptop-day; 11 modifications passed the keep/discard gate (yielding 12 kept policies when counting the initial baseline).
Its final policy combines an oracle consensus voting stage with a learned reranker \citep{burges2010ranknet,ke2017lightgbm}; this structure was discovered automatically rather than prescribed by the authors.
...
A set of 3{,}554 peptides scored across activity, toxicity, stability, and developability, evaluated under three mathematically distinct oracle definitions, is released so other groups can benchmark their own candidate prioritization methods on a common target.

\item \textbf{Evidence that automated policy search beats standard baselines on candidate prioritization.}
...
\item \textbf{Evidence of policy-structure discovery beyond random weight search.}
