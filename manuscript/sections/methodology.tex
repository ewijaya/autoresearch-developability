...
Given a pool of $N$ peptide candidates, each characterized by estimated scores for $K$ endpoints (activity, toxicity, stability, developability), the goal is to learn a ranking policy $\pi$ that maps score vectors to a candidate ordering maximizing discovery utility under all constraints simultaneously.
...
A deterministic rule-based proxy computes a penalty score from seven sequence-level rules \citep{fjell2012peptidedev}: length outside 8--40 residues ($+1$), consecutive hydrophobic stretches ($+0.5$ per excess), consecutive prolines ($+0.5$), high net charge ($+1$), high mean hydrophobicity ($+1$), and excess cysteines ($+1$).
