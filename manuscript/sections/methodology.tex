...
We formalize the ranking problem, describe the public benchmark and evaluation harness, and detail the autoresearch loop and baselines.

\subsection{Problem Formulation}
\label{sec:problem}

Given a pool of $N$ peptide candidates, each characterized by estimated scores for four endpoints (activity, toxicity, stability, developability), the goal is to learn a ranking policy $\pi$ that maps score vectors to a candidate ordering maximizing discovery utility under all constraints simultaneously.
...
\midrule
Activity & DBAASP v3 & 3{,}554 & Measured MIC & --- & $-\log_{10}$ potency \\
...
Developability & Rule-based & --- & 6 penalties & Deterministic & Length, charge, hydrophobicity \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Activity.}
Antimicrobial activity is derived from minimum inhibitory concentration (MIC) values in the Database of Antimicrobial Activity and Structure of Peptides (DBAASP v3) \citep{pirtskhalava2021dbaasp}, filtered to \emph{E.~coli} ATCC 25922 as target strain.
...
A deterministic rule-based proxy computes a penalty score from six sequence-level rules \citep{fjell2012peptidedev}: length outside 8--40 residues ($+1$), consecutive hydrophobic stretches ($+0.5$ per excess), consecutive prolines ($+0.5$), high net charge ($+1$), high mean hydrophobicity ($+1$), and excess cysteines ($+1$).
...
We adapt the autoresearch framework \citep{karpathy2026autoresearch} from neural architecture search to ranking policy search (see Section~\ref{sec:related_work} for the companion paper).
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Random weight search}: following the random-search-as-baseline paradigm \citep{bergstra2012random}, we sample 1{,}000 Dirichlet weight vectors and evaluate each with the weighted-sum scalarization. This controls for blind exploration of the same policy class the agent starts from.
