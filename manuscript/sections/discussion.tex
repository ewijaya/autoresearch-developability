...
The agent's 12 kept improvements follow a clear structural trajectory.
...
The agent independently discovered oracle consensus voting---prioritizing candidates that appear in at least two of three independent top-$k$ shortlists---a form of ensemble agreement selection that the authors did not pre-specify.

\textbf{Agent search vs.\ random search.}
The agent outperforms best-of-1{,}000 random weight search (0.650 vs.\ 0.611 top-$k$ enrichment), confirming that directed policy search adds value beyond blind exploration.
Critically, the agent escapes the linear scalarization policy class entirely: consensus voting and learned reranking are structural innovations that no amount of random Dirichlet weight sampling can discover.
...
\textbf{Portability.}
Despite these limitations, the framework is designed for straightforward adaptation to proprietary data.
